...
export type GameStatus = "lobby" | "playing" | "paused" | "over";
...
export function setModeration(roomId: string, room: Room, uid: string, patch: Moderation) {
  const moderation = room.moderation ?? {};
  return updateRoom(roomId, { moderation: { ...moderation, [uid]: { ...moderation[uid], ...patch } } });
}
